...
def update():
    ...
    try:
        ...
        value_chain = []
        # value_chain = ser.getData()  # I think this is the command that gets data from the serial (communication.py)
        # value_chain = sensors.read_sensor()  # This should read things from the sensors themselves
        # update_pressure1(data_group(p_sensors1))  # This is grouping the data from sensor array 1, updating in GUI
        # update_pressure2(data_group(p_sensors2))  # Should be noted that data_group is the grouping function, and that
        # update_temp(data_group(t_sensors))        # the inputs are dictionaries (containing individual sensors)
